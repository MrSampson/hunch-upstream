...
This repo has **Hunch** — a curated graph of *why* the code is the way it is (decisions, bug history, invariants). It currently holds **327 decisions, 2 bugs, 31 constraints, 22 components, 3 policies, 13 open findings**.
...
- **[warning]** Memory hygiene is the agent's job, done without waiting for the human: accept shipped roadmap proposals, settle provenance-repair escalations (apply the true commit or drop), re-anchor or stale findings, and land the resulting capture commits through a PR. Only decisions that need genuine product judgment are queued for the human. _(scope: .hunch/**, ROADMAP.md, docs/autonomous-development.md; con_039cee7367)_
