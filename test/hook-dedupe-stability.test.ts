...
test("the identity projection drops presentation and keeps records: order is not a change, membership is", () => {
  ...
  const project = (s: readonly DeliverySupplement[]) => deliveryDedupeInput(buildDeliveryEnvelope(ctx, { supplements: s }), s);
  ...
  // No hash_text anywhere → the projection is the rendered text (old contract).
  const plain = [{ id: "x", kind: "note", priority: 10, text: "a plain supplement" }];
  const envelope = buildDeliveryEnvelope(ctx, { supplements: plain });
  assert.equal(deliveryDedupeInput(envelope, plain), envelope.text);
