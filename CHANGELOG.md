...
- The pre-edit hook stops re-sending a full grounding block that nothing changed: deduplication now hashes the identity of the records behind the block rather than its rendered text, so delivery receipts, ranking warmth, slot labels and relative ages ("today" → "delivered today") no longer invalidate it, while a record entering, leaving or changing content still does. Measured 2026-09-19 in this repository's own sessions: 63 of 169 full pre-edit injections in subagents (about 3.8k characters each) were repeats for the same agent and file. Nothing is removed from what the agent sees, and an unexpected input still falls back to the old behaviour — a full block, never a wrong delta (#373).
- The native tree-sitter addons load on first parse instead of at import, so a CLI process that never parses no longer copies and `dlopen`s six native addons at startup. Measured over 5 interleaved runs (min/median): `hunch --version` 1.53/1.92s → 0.24/0.25s, `hunch now` 1.89/1.97s → 0.51/0.52s, `hunch doctor` 2.70/3.20s → 1.47/1.52s, and the Claude Code PreToolUse hook 2.16/2.60s → 0.79/0.89s. A parser that cannot load now fails loudly instead of quietly: `hunch index` and `hunch init` abort rather than marking every file unparseable and replacing the graph with empty symbols, edges and components while exiting 0; `hunch worktree` keeps the worktree and reports the skipped index; and `hunch doctor` gains a `parser:` check that says so (#366).
- Kubernetes manifest references resolve within a namespace: two same-named ConfigMaps in different namespaces no longer make every reference ambiguous, and a lone same-named resource in another namespace is no longer linked as if it were local. A missing, templated or unparseable namespace is unknown and matches anything; only two different literal namespaces block an edge. A `backendRefs[].namespace` sibling on an HTTPRoute overrides the document's namespace, and a document whose several `metadata.namespace` entries disagree (a Helm if/else) reads as unknown (closes #297).
- `hunch_why`, `hunch_structure`, `hunch_get_dependents`, `hunch_check_constraints`, `hunch_blast_radius`, the pre-edit hook, findings and task delivery share one definition of a known path, so a target can no longer pick up another file's records: a root `index.ts` no longer returns `a/index.ts`'s symbols, an absolute target resolves instead of matching nothing, and a real working-tree file with no symbols and no covering component is no longer matched by basename to a same-named indexed file. `hunch_structure` on a real file with no symbols says so instead of calling it "not a known file", and an in-repo symlink pointing outside the repository cannot report an outside file as a repo file (closes #334, #335).
- A `hunch init` run from inside a linked worktree keeps the repository's shared git hooks instead of re-pointing every checkout at a launcher that disappears with `git worktree remove`. A healthy shared block is kept as-is, a missing or stale one is rebuilt from a healthy sibling's launcher, and a kept block never loses its own option flags — so a strict guard is not silently downgraded to advisory. A re-run writes the union of what the block already carries and what the run asks for, so a requested `--private` lands instead of being reported as kept and never written, and a block with no closing marker is reported rather than passed off as unchanged (#356).
- A `<!-- hunch:topic … -->` marker shown as an example inside a code fence that is indented under a list item (`1. step`) is left alone: `hunch drift` and the pre-edit hook no longer read it as a live pinned anchor and report false `doc-anchor-stale` or `doc-anchor-dangling` drift. An unclosed fence now ends with its list item instead of swallowing every real anchor to the end of the file, and a line made of many list markers no longer takes seconds to scan (closes #331).
- Merging two clones' state ledgers no longer hides events from a subscriber: when the merge renumbers an event either side had already published, the floor is raised past both heads so every pre-merge cursor resyncs instead of sitting above an event that now carries a lower sequence number. A merge that renumbers nothing keeps its sequence numbers and its floor (closes #285).
